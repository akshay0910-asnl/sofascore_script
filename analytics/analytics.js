...
function probabilityOverAndUnderThreshold(data, config) {
    ...
    allMatches.forEach(match => {
        Object.entries(metricsConfig).forEach(([metric, config]) => {
            ...
            let value = metricScope === "team" ? team : metricScope === "opponent" ? opponent : team + opponent;
            ...
            gteThresholds.forEach(threshold => {
                result.metrics[metric][`>=${threshold}`] = {
                    total: {
                        ...
                        probability: Number((hitsByMetric[metric].gte[threshold].total / totalCount).toFixed(4)),
                    },
                    home: {
                        hits: hitsByMetric[metric].gte[threshold].home,
                        totalMatches: homeCount,
                        probability: homeCount ? Number((hitsByMetric[metric].gte[threshold].home / homeCount).toFixed(4)) : 0,
                    },
                    away: {
                        hits: hitsByMetric[metric].gte[threshold].away,
                        totalMatches: awayCount,
                        probability: awayCount ? Number((hitsByMetric[metric].gte[threshold].away / awayCount).toFixed(4)) : 0,
                    },
                };
            });
        }
            ...
            ltThresholds.forEach(threshold => {
                result.metrics[metric][`<${threshold}`] = {
                    total: {
                        ...
                        totalMatches: totalCount,
                        probability: Number((hitsByMetric[metric].lt[threshold].total / totalCount).toFixed(4)),
                    },
                    home: {
                        hits: hitsByMetric[metric].lt[threshold].home,
                        totalMatches: homeCount,
                        probability: homeCount ? Number((hitsByMetric[metric].lt[threshold].home / homeCount).toFixed(4)) : 0,
                    },
                    away: {
                        hits: hitsByMetric[metric].lt[threshold].away,
                        totalMatches: awayCount,
                        probability: awayCount ? Number((hitsByMetric[metric].lt[threshold].away / awayCount).toFixed(4)) : 0,
                    },
                };
            });
        }
...
async function probabilityOverAndUnderThresholdWrapper(data, teamSlug) {
    ...
    await fs.writeFile(`./teamsData/${teamSlug}_analysis.json`, JSON.stringify(result, null, 2));
}
