...
const {
  probabilityOverAndUnderThresholdWrapper,
...
const CONFIG = {
  ...
  buttons: {
    nextButton:
      ".md\\:bg_surface\\.s1.card-component button.enabled\\:active\\:bg_primary\\.highlight.enabled\\:focusVisible\\:bg_primary\\.highlight",
  },
};
...
function transformMatchStatistics(data, matchData) {
  const matchStats = {
    ...
    goals: matchData.isHome
      ? `${matchData.goals[0]}/${matchData.goals[1]}[H]`
      : `${matchData.goals[1]}/${matchData.goals[0]}[A]`,
    throwIns: `0/0${matchData.isHome ? "[H]" : "[A]"}`,
    averageBallPossession: `0/0${matchData.isHome ? "[H]" : "[A]"}`,
    teams: matchData.isHome
      ? [matchData.teams[0], matchData.teams[1], "Home"].join(",")
      : [matchData.teams[1], matchData.teams[0], "Away"].join(","),
  ...
  for (const group of allStatistics.groups) {
    ...
    for (const item of statisticsItems) {
      ...
      switch (name) {
        case "Corner kicks": {
          matchStats.corners = matchData.isHome
            ? `${homeValue || 0}/${awayValue || 0}[H]`
            : `${awayValue || 0}/${homeValue || 0}[A]`;
          break;
        }
        case `Ball possession`: {
          matchStats.averageBallPossession = matchData.isHome
            ? `${homeValue || 0}/${awayValue || 0}[H]`
            : `${awayValue || 0}/${homeValue || 0}[A]`;
          break;
        }
        case `Fouls`: {
          matchStats.fouls = matchData.isHome
            ? `${homeValue || 0}/${awayValue || 0}[H]`
            : `${awayValue || 0}/${homeValue || 0}[A]`;
          break;
        }
        case `Goal kicks`: {
          matchStats.goalKicks = matchData.isHome
            ? `${homeValue || 0}/${awayValue || 0}[H]`
            : `${awayValue || 0}/${homeValue || 0}[A]`;
          break;
        }
        case `Total saves`: {
          matchStats.saves = matchData.isHome
            ? `${homeValue || 0}/${awayValue || 0}[H]`
            : `${awayValue || 0}/${homeValue || 0}[A]`;
          break;
        }
        case `Offsides`: {
          matchStats.offsides = matchData.isHome
            ? `${homeValue || 0}/${awayValue || 0}[H]`
            : `${awayValue || 0}/${homeValue || 0}[A]`;
          break;
        }
        case `Shots on target`: {
          matchStats.shotsOnTarget = matchData.isHome
            ? `${homeValue || 0}/${awayValue || 0}[H]`
            : `${awayValue || 0}/${homeValue || 0}[A]`;
          break;
        }
        case `Yellow cards`: {
          matchStats.yellowCards = matchData.isHome
            ? `${homeValue || 0 || 0}/${awayValue || 0 || 0}[H]`
            : `${awayValue || 0 || 0}/${homeValue || 0 || 0}[A]`;
          break;
        }
        case `Throw-ins`: {
          matchStats.throwIns = matchData.isHome
            ? `${homeValue || 0}/${awayValue || 0}[H]`
            : `${awayValue || 0}/${homeValue || 0}[A]`;
          break;
        }
        case `Tackles`: {
          matchStats.tackles = matchData.isHome
            ? `${homeValue || 0}/${awayValue || 0}[H]`
            : `${awayValue || 0}/${homeValue || 0}[A]`;
          break;
        }
        case `Interceptions`: {
          matchStats.interceptions = matchData.isHome
            ? `${homeValue || 0}/${awayValue || 0}[H]`
            : `${awayValue || 0}/${homeValue || 0}[A]`;
          break;
        }
        case `Aerial duels`: {
          matchStats.aerialDuels = matchData.isHome
            ? `${homeValue || 0}/${awayValue || 0}[H]`
            : `${awayValue || 0}/${homeValue || 0}[A]`;
          matchStats.aerialDuelsAttempted = matchData.isHome
            ? `${homeTotal || 0}/${awayTotal || 0}[H]`
            : `${awayTotal || 0}/${homeTotal || 0}[A]`;
          break;
        }
        case "Dribbles": {
          matchStats.dribbles = matchData.isHome
            ? `${homeValue || 0}/${awayValue || 0}[H]`
            : `${awayValue || 0}/${homeValue || 0}[A]`;
          matchStats.dribblesAttempted = matchData.isHome
            ? `${homeTotal || 0}/${awayTotal || 0}[H]`
            : `${awayTotal || 0}/${homeTotal || 0}[A]`;
          break;
        }
...
function parseCombinedStatistics(teamId) {
  ...
  const resultModified = Object.entries(intermediate).reduce(
    (acc, [key, value]) => {
      const newValue = value.split("+");
      let numeratorCount = 0,
        denominatorCount = 0,
        count = 0,
        numeratorCountHome = 0,
        denominatorCountHome = 0,
        countHome = 0,
        numeratorCountAway = 0,
        denominatorCountAway = 0,
        countAway = 0;
      ...
      if (key !== "teams" && key !== "isHome") {
        ...
        acc[`${key}TotalsHome`] =
          `${numeratorCountHome}/${denominatorCountHome}`;
        acc[`${key}TotalsAway`] =
          `${numeratorCountAway}/${denominatorCountAway}`;
        acc[`${key}Average`] =
          `${(numeratorCount / count).toFixed(2)}/${(denominatorCount / count).toFixed(2)}`;
        acc[`${key}AverageHome`] =
          `${(numeratorCountHome / countHome).toFixed(2)}/${(denominatorCountHome / countHome).toFixed(2)}`;
        acc[`${key}AverageAway`] =
          `${(numeratorCountAway / countAway).toFixed(2)}/${(denominatorCountAway / countAway).toFixed(2)}`;
      }
      return acc;
    },
    { ...intermediate },
  ...
  cache = {
    ...
    [teamId]: {
      ...cache[teamId],
      combinedMatchStatistics: { ...resultModified },
    },
  };
...
const handleStatisticsResponse = async (response, teamId) => {
  ...
  if (url.includes("/api/v1/team/") && url.includes("/statistics/")) {
    try {
      ...
      if (!response.ok()) {
        console.warn(
          `⚠ Statistics response returned status ${response.status()}`,
        );
        return;
      }
  ...
  } else if (url.includes("/api/v1/team/") && url.includes("/events/last/0")) {
    try {
      // Check if response is OK before attempting to parse JSON
      if (!response.ok()) {
        console.warn(`⚠ Events response returned status ${response.status()}`);
        return;
      }
      const result = await response.json();
      cache[teamId].recentEvents = [...result.events.reverse()];
    } catch (err) {
...
const handleMatchStatisticsResponse = async (
  response,
  match,
  index,
  page,
  ...
  if (
    ...
    try {
      ...
      if (!response.ok()) {
        console.warn(
          `⚠ Match statistics response returned status ${response.status()} for match ${matchId}`,
        );
        return;
      }
...
async function setUpMatchStatisticsInterception(
  page,
  match,
  index,
  teamId,
...
async function navigateToPage(page, url, teamId) {
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      console.error(
        `✗ Page load error (attempt ${attempt}/${3}):`,
        err.message,
      );
      ...
      if (attempt === 3) {
        ...
        throw new Error(
          `Failed to load page after ${3} attempts: ${err.message}`,
        );
      }
...
async function navigateToMatchPage(page, match, index, teamId, parentPage) {
  try {
    await setUpMatchStatisticsInterception(
      page,
      match,
      index,
      teamId,
...
async function extractTeamInfo(page, teamId) {
  try {
    ...
    if (match) {
      ...
      console.log(
        `✓ Team info extracted: ${cache[teamId].teamSlug} (ID: ${cache[teamId].teamId})`,
      );
    }
  } catch (err) {
    ...
    for (const data of scripts) {
      if (
        ...
        cache[teamId].teamName =
          data.itemListElement[data.itemListElement.length - 1].name;
...
async function extractMatchScore(linkElement, index, teamId) {
  try {
    ...
    try {
      ...
      console.error(
        `⚠ Error extracting teams from match ${index}:`,
        err.message,
      );
    }

    let teamIds = [];
    const images = await linkElement
      .$$(CONFIG.selectors.teamImg)
      .catch(() => null);
    try {
      ...
      console.error(
        `⚠ Error extracting team IDs from match ${index}:`,
        err.message,
      );
    }
    ...
    try {
      ...
      console.error(
        `⚠ Error extracting scores from match ${index}:`,
        err.message,
      );
    }
    ...
    const teamIndex = teamIds.findIndex(
      (teamId1) => teamId1 == cache[teamId].teamId,
    ); //teams.indexOf(cache[teamId].teamName);
    let isHome;
    if (teamIndex === 0) {
    ...
    console.log(
      `⚽ Match ${index}: ${teams.join(" vs ")} - Score: ${scores.join("-")}`,
    );
    ...
    cache = {
      ...
      [teamId]: {
        ...cache[teamId],
        matchLinks: [
          ...cache[teamId].matchLinks,
          {
            href: `https://www.sofascore.com${href},tab:statistics`,
            teams,
            goals: scores.filter((el, index) => index % 2 === 0),
            teamIds,
            isHome,
          },
        ],
...
async function extractTournamentMatches(page, index, teamId) {
  ...
  try {
    ...
    console.error(
      `✗ Error extracting tournament matches [${index}]:`,
      err.message,
    );
    return null;
  }
...
async function processAllMatchesNew(page, context, teamId) {
  ...
  for (const event of cache[teamId].recentEvents) {
    ...
    const teams = [homeTeamName, awayTeamName];
    ...
    // Extract scores
    const homeScore = event.homeScore.normaltime;
    const awayScore = event.awayScore.normaltime;
    const goals = [homeScore, awayScore];

    //Extract custom id
    const customId = event.customId;

    // Determine if home or away match
    const teamIndex = teamIds.findIndex((id) => id == cache[teamId].teamId);
    let isHome;

    if (teamIndex === 0) {
      if (cache[teamId].homeMatchCount >= 4) {
        continue;
      }
      cache[teamId].homeMatchCount += 1;
      isHome = true;
    } else if (teamIndex === 1) {
      if (cache[teamId].awayMatchCount >= 4) {
        continue;
      }
      cache[teamId].awayMatchCount += 1;
      isHome = false;
    ...
    // Construct match link
    const href = `https://www.sofascore.com/football/match/${event.slug}/${customId}#id:${event.id},tab:statistics`;

    cache[teamId].matchLinks.push({
      href,
      teams,
      goals,
      teamIds,
    ...
    console.log(`⚽ Match: ${teams.join(" vs ")} - Score: ${goals.join("-")}`);

    if (
      cache[teamId].homeMatchCount >= 4 &&
      cache[teamId].awayMatchCount >= 4
    ) {
      break;
    }
  }

  // After processing events, open match links if available
  if (cache[teamId].matchLinks.length > 0) {
...
async function processAllMatches(page, context, teamId) {
  try {
    ...
    const tournamentElements = await page.$$(
      CONFIG.selectors.tournamentElement,
    );
    ...
    if (nextButtons.length >= 2) {
      ...
      if (
        !(
          cache[teamId].homeMatchCount >= 4 && cache[teamId].awayMatchCount >= 4
        )
      ) {
    ...
    for (let j = i; j < Math.min(i + 2, cache[teamId].matchLinks.length); j++) {
      ...
      const batchPromise = (async () => {
        try {
          ...
          console.error(
            `✗ Error in match index ${absoluteIndex}:`,
            err.message,
          );
        }
      })();
    ...
    try {
      const interval = setInterval(async () => {
        ...
        if (allCollected) {
          cache[teamId].endTime = new Date();
          cache[teamId].totalTimeSeconds = parseFloat(
            ((cache[teamId].endTime - cache[teamId].startTime) / 1000).toFixed(
              2,
            ),
          );
          console.log("\n✓ All data collected. Closing browser.");
...
function isAllDataCollected(teamId) {
  ...
  // }
  if (
    cache[teamId]?.matchStatistics?.length !== cache[teamId]?.matchLinks?.length
  ) {
    return false;
  }
  ...
  const homeMatchesWithDataAvailable = cache[teamId].matchStatistics.filter(
    (stat) => stat && stat.isHome,
  ).length;
  const awayMatchesWithDataAvailable = cache[teamId].matchStatistics.filter(
    (stat) => stat && !stat.isHome,
  ).length;
...
async function writeStatisticsToFile(teamId) {
  ...
  try {
    ...
    await probabilityOverAndUnderThresholdWrapper(
      cache[teamId],
      cache[teamId].teamSlug,
    );
